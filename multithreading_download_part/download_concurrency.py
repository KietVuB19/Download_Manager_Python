...
class Downloader:
    ...
    def __init__(self, url=None, number_of_threads=1):
        ...
        self.q = Queue(maxsize=0)  # worker threads will pick download job from the queue
    ...
    def get_file_size(self):
        ...
        :return: integer
        """
        self.file_size = requests.head(self.url, headers={'Accept-Encoding': 'identity'}).headers.get('content-length',
                                                                                                      None)
    ...
    def display_benchmarks(self):
        ...
        print("\nTotal time taken for download and integrity check:", round(self.end_time - self.start_time, 2),
              "seconds.")
        if self.if_byte_range:
